...
public class EnergyManager : IDisposable
{
    ...
    {
        return _computer.Hardware.Sum(hardware =>
            hardware.Sensors.Where(sensor => sensor.SensorType == SensorType.Power)
                .Select(powerSensor => powerSensor.Value.GetValueOrDefault(0)).Sum());
    }
    ...
    public List<float> GetCpuUsages()
    {
        ...
        foreach (var usageSensors in cpuList.Select(cpu =>
                     cpu.Sensors.Where(sensor => sensor.Name.Contains("CPU Total"))))
    ...
    public float GetRamUsage()
    {
        ...
        var ramUsageSensor =
            ram?.Sensors.FirstOrDefault(ramSensor => ramSensor is { SensorType: SensorType.Load, Name: "Memory" });
    ...
    public List<float> GetDiskUsages()
    {
        ...
        foreach (var diskSensors in diskList.Select(diskSensor =>
                     diskSensor.Sensors.Where(sensor => sensor is { SensorType: SensorType.Load, Name: "Used Space" })))
    ...
    public List<Dictionary<string, float>> GetEthernetUsages()
    {
        ...
        return ethernetList
            .Select(ethernetSensor =>
                ethernetSensor.Sensors.Where(sensor => sensor.SensorType == SensorType.Throughput))
            .Select(ethernetSensors => ethernetSensors.ToDictionary(x => x.Name, x => x.Value.GetValueOrDefault(0)))
            .ToList();
    }
